# class User - объект пользователя имеющий свои данные и свой функционал, может взаимодействовать только с самим с собой, вводимые данные характерны для всех
#пользователей вне зависимости от статуса ("user","moderator","admin")
# status и blocking - постоянные значения при создании класса, могут быть измененны только class Moderator и Admin
class User(): 
...
# class Admin имеет свойства и методы как у class Moderator, но имеет доп метод удаления/создания всех users и status Admin, blocking наследуется от User (False)
